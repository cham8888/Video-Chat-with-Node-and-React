...
const createSocketHanlder = (io) => {
  ...
  return function socketHandler(socket) {
    ...
    socket.on('joinUser', async (userName, password, type, roomId, version, callback) => {
      ...
      // check username
      const temp_user = users.getUserByUsername(userName);
      if(temp_user) {
        return callback('Username taken');
      }

      // check user
      if (type == 'server_auto_login') {
      ...
      try {
        if (user_token.user_type == 'guest') {
          ...
          if (user_token.type == 'server_auto_login') {
            decodedUsername = utf8.decode(base64.decode(user_token.userName));
            decodedPassword = utf8.decode(base64.decode(user_token.password));
          }
          // try {
          ...
          // end check user from server request
        }
      } catch (err) {
        console.log({ err });
        return callback(err);
